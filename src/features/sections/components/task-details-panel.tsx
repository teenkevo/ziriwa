...
export function TaskDetailsPanel({
  ...
  const inputsApproved = task
    ...
    : false
  const inputsLocked = isKPI && !expectedDeliverableSet
  ...
  const handleConfirmDeliverable = () => {
    onUpdate({ expectedDeliverable: deliverableEditValue.trim() || undefined })
    setWaitingForSave(true)
  }
              ...
              {taskFreq !== 'n/a' && (
                <CardContent className='pt-0 space-y-4'>
                        ...
                        <PopoverTrigger asChild>
                          ...
                          >
                            <span className='flex items-center gap-2'>
                              ...
                              {task.reportingPeriodStart ? (
                                format(
                                  parseDateAsLocal(task.reportingPeriodStart),
                                  'PPP',
                                )
                              ) : (
                                <span>Defaults to FY start</span>
                              )}
                            </span>
                          </Button>
                        </PopoverTrigger>
                  ...
                  <div className='space-y-2'>
                    ...
                    {isEditingDeliverable ? (
                      <div ref={deliverableEditRef} className='space-y-2 mt-1'>
                        <textarea
                          ...
                          onKeyDown={e => {
                            if (e.key === 'Escape') handleCancelDeliverable()
                          }}
                          autoFocus
            ...
            <div className='space-y-1'>
              ...
              {inputsLocked && (
                <p className='text-xs text-muted-foreground'>
                  Specify the expected deliverable to allow submission of
                  inputs.
                </p>
              )}
              {(() => {
                        ...
                        <div className='flex items-center gap-3 p-2 rounded-md border bg-muted/30'>
                          ...
                          <div className='flex-1 space-y-1 min-w-0'>
                            <p className='text-sm font-medium truncate'>
                              {approvedFile.originalFilename ?? 'inputs.pdf'}
                            </p>
                      ...
                      >
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Upload a new inputs file and add a comment for the
                              supervisor.
                            </DialogDescription>
                          </DialogHeader>
                          <div className='py-4 space-y-3'>
                            <div>
                              ...
                              <input
                                ...
                                onChange={e => {
                                  setResubmitFile(e.target.files?.[0] ?? null)
                                }}
                              />
                              <div className='mt-1 flex items-center gap-2'>
                                <Button
                      ...
                      <div className='rounded-md border divide-y max-h-[180px] overflow-y-auto'>
                        ...
                        !task.inputs?.file?.asset?.url ? (
                          ...
                          [...(task.inputsReviewThread ?? [])]
                            ...
                            .map((entry, i) => {
                              const origIdx =
                                (task.inputsReviewThread ?? []).length - 1 - i
                              const fileToShow =
                                entry.file?.asset ??
                                (entry.action === 'submit'
                                  ? task.inputs?.file?.asset
                ...
                <TabsContent value='main' className='space-y-2 mt-2'>
                  ...
                  {(task.status === 'in_review' || task.status === 'done') &&
                    (task.deliverable ?? []).some(
                      ...
                      <div className=' space-y-3'>
                        {(() => {
                          const thread = task.deliverableReviewThread ?? []
                          const mainDeliverable = (task.deliverable ?? []).find(
                            e => (e.tag ?? 'support') === 'main',
                          )
                          const mainFile = mainDeliverable?.file?.asset
                          const submitEntry = thread.find(
                            e => e.action === 'submit' && e.role === 'officer',
                          )
                                      ...
                                      {!needsOfficerResubmit &&
                                        ...
                                        onRejectDeliverable && (
                                          <div className='flex flex-wrap gap-2 pt-1'>
                                            ...
                                            <Dialog
                                              open={deliverableRejectDialogOpen}
                                              onOpenChange={open => {
                                                if (
                                                  !open &&
                                                  (isSaving ||
                                ...
                                {!needsOfficerResubmit &&
                                  ...
                                  onRejectDeliverable && (
                                    <div className='flex flex-wrap gap-2'>
                                      <Dialog
                                        open={deliverableApproveDialogOpen}
                                        onOpenChange={open => {
                                          if (
                                            !open &&
                                            (isSaving ||
                                            ...
                                            return
                                          setDeliverableApproveDialogOpen(open)
                                          if (!open)
                                            setDeliverableApproveReason('')
                                        }}
                                      >
                                        ...
                                        <DialogContent>
                                          <DialogHeader>
                                            ...
                                            <DialogDescription>
                                              Approval will mark the task as
                                              Done.
                                            </DialogDescription>
                                          </DialogHeader>
                                          <div className='py-4'>
                                            ...
                                            <textarea
                                              value={deliverableApproveReason}
                                              onChange={e =>
                                                setDeliverableApproveReason(
                                                  e.target.value,
                                                )
                                      ...
                                      <Dialog
                                        ...
                                        onOpenChange={open => {
                                          if (
                                            ...
                                            return
                                          setDeliverableRejectDialogOpen(open)
                                          if (!open)
                                            setDeliverableRejectMessage('')
                                        }}
                                      >
                                        ...
                                        <DialogContent>
                                          ...
                                          <div className='py-4'>
                                            ...
                                            <textarea
                                              value={deliverableRejectMessage}
                                              onChange={e =>
                                                setDeliverableRejectMessage(
                                                  e.target.value,
                                                )
                          ...
                          if (isDone) {
                            if (thread.length === 0 && !mainFile) return null
                            const approvedFile =
                              mainFile ??
                              [...thread]
                                .reverse()
                            ...
                            <>
                              {needsOfficerResubmit && (
                                <Dialog
                                  ...
                                  onOpenChange={open => {
                                    if (!open && deliverableResubmitting) return
                                    setDeliverableResubmitDialogOpen(open)
                                    if (!open) {
                                ...
                                >
                                  <DialogContent>
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        Upload a replacement file and add a note
                                        for the supervisor.
                                      </DialogDescription>
                                    </DialogHeader>
                                    <input
                                      ...
                                      onChange={e => {
                                        ...
                                        setDeliverableResubmitFile(f ?? null)
                                      }}
                                    />
                                    <div className='space-y-4 py-4'>
                                      ...
                                      <div>
                                        ...
                                        <textarea
                                          value={deliverableResubmitComment}
                                          onChange={e =>
                                            setDeliverableResubmitComment(
                                              e.target.value,
                                            )
                                          ...
                                          disabled={
                                            isSaving || deliverableResubmitting
                                          }
                                          className='mt-1 w-full min-h-[80px] rounded-md border bg-background px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50'
                                    ...
                                    <DialogFooter>
                                      <Button
                                        ...
                                        disabled={deliverableResubmitting}
                                      >
                                        Cancel
                                      </Button>
                                      <Button
                                        onClick={async () => {
                                          if (!onRespondToDeliverableRejection)
                                            return
                                          setDeliverableResubmitting(true)
                                          try {
                                            ...
                                            setDeliverableResubmitting(false)
                                          }
                                        }}
                                        disabled={
                                          isSaving ||
