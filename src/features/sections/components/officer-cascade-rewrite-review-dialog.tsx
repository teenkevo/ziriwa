...
interface OfficerCascadeRewriteReviewDialogProps {
  open: boolean
...
export function OfficerCascadeRewriteReviewDialog({
  open,
  onOpenChange,
  items,
  drafts,
  ...
  onConfirmImport,
  onBack,
}: OfficerCascadeRewriteReviewDialogProps) {
  const groupedItems = React.useMemo(
    () => groupItemsByInitiative(items),
    [items],
  )
  const totalTaskCount = React.useMemo(
    () => items.reduce((n, item) => n + item.asIs.tasks.length, 0),
    [items],
  )
            ...
            return (
              ...
              >
                ...
                <div className='space-y-2'>
                  <Label htmlFor={`officer-objective-${initiativeKey}`}>
                    Your SSMARTA objective
                  </Label>
                  <Textarea
                    id={`officer-objective-${initiativeKey}`}
                ...
                {groupItems.map(item => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className='space-y-2'>
                        <Label htmlFor={`officer-initiative-${item.activityKey}`}>
                          Your initiative
                        </Label>
                        <Textarea
                          id={`officer-initiative-${item.activityKey}`}
                      ...
                      <div className='space-y-2'>
                        <Label htmlFor={`officer-measurable-${item.activityKey}`}>
                          Your measurable activity
                        </Label>
                        <Textarea
                          id={`officer-measurable-${item.activityKey}`}
                      ...
                      <div className='space-y-2'>
                        <Label htmlFor={`officer-tasks-${item.activityKey}`}>
                          Detailed tasks (one per line)
                        </Label>
                        <Textarea
                          id={`officer-tasks-${item.activityKey}`}
        ...
        <DialogFooter className='gap-2 sm:gap-0'>
          ...
          <Button type='button' onClick={onConfirmImport} disabled={isSubmitting}>
            {isSubmitting ? (
              ...
              `Import ${totalTaskCount} task${totalTaskCount === 1 ? '' : 's'}`
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
