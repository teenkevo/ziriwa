...
export async function PATCH(
  ...
  try {
    ...
    const projectId = await getProjectIdFromContract(id)
    if (!projectId) {
      return NextResponse.json({ error: 'Contract not found' }, { status: 404 })
    }
    const contractOpDenied =
      await assertProjectContractManageAllowed(projectId)
    if (contractOpDenied) return contractOpDenied

    const contractLabel = await client.fetch<string | null>(
      `coalesce(*[_type == "projectContract" && _id == $id][0].financialYearLabel, "Project contract")`,
      { id },
    )
