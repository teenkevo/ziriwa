...
export async function PATCH(
  ...
  try {
    ...
    const divisionId = await getDivisionIdFromContract(id)
    if (!divisionId) {
      return NextResponse.json({ error: 'Contract not found' }, { status: 404 })
    }
    const contractOpDenied =
      await assertDivisionContractManageAllowed(divisionId)
    ...
    console.error('Error patching division contract', error)
    return NextResponse.json(
      { error: 'Failed to update division contract' },
      { status: 500 },
    )
  }
}
