...
import {
  ...
  getProjectIdFromDeputyContract,
} from '@/lib/deputy-project-contract-access.server'
import { client } from '@/sanity/lib/client'

/**
 * PATCH /api/project-contracts/[id] - Add objective, initiative, or activity
...
export async function PATCH(
  ...
  try {
    ...
    const projectId = await getProjectIdFromDeputyContract(id)
    if (!projectId) {
      return NextResponse.json({ error: 'Contract not found' }, { status: 404 })
    }
    const contractOpDenied =
      await assertDeputyProjectContractManageAllowed(projectId)
    if (contractOpDenied) return contractOpDenied

    const contractLabel = await client.fetch<string | null>(
      `coalesce(*[_type == "deputyProjectContract" && _id == $id][0].financialYearLabel, "Project contract")`,
      { id },
    )
    ...
    console.error('Error patching project contract', error)
    return NextResponse.json(
      { error: 'Failed to update project contract' },
      { status: 500 },
    )
  }
}
