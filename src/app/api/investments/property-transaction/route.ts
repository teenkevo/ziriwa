  ...
  try {
    ...
    if (!investmentId || !transactionType || !amountStr || !date) {
      return NextResponse.json(
        {
          error:
            'Missing required fields: investmentId, transactionType, amount, date',
        },
        { status: 400 },
      )
    }
    ...
    if (requiresDocs && (!documentFiles || documentFiles.length === 0)) {
      return NextResponse.json(
        {
          error: 'Supporting documents (title deed, sale agreement) are required for purchase and sale transactions',
        },
        { status: 400 },
      )
    }
