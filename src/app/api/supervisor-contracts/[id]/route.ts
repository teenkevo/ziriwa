...
/**
 * PATCH /api/supervisor-contracts/[id] - Add objective, initiative, or activity
...
export async function PATCH(
  ...
  try {
    ...
    const sectionId = await getSectionIdFromSupervisorContract(id)
    if (!sectionId) {
    ...
    console.error('Error patching supervisor contract', error)
    return NextResponse.json(
      { error: 'Failed to update supervisor contract' },
      { status: 500 },
    )
  }
}
