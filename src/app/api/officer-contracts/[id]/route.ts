...
export async function PATCH(
  ...
  try {
    ...
    const contractOpDenied = await assertOfficerContractManageAllowed(
      sectionId,
    ...
    console.error('Error patching officer contract', error)
    return NextResponse.json(
      { error: 'Failed to update officer contract' },
      { status: 500 },
    )
  }
}
